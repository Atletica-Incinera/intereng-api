...
describe('getOrCreateRequestId', () => {
  ...
  it('should use req.id if it is already present as a string', () => {
    ...
    const result = getOrCreateRequestId(mockReq as IncomingMessage, mockRes as ServerResponse);

    expect(result).toBe('existing-req-id');
    expect(mockReq.headers?.['x-request-id']).toBe('existing-req-id');
    expect(mockRes.setHeader).toHaveBeenCalledWith('x-request-id', 'existing-req-id');
  });
  ...
  it('should fallback to x-request-id header if req.id is not present', () => {
    ...
    expect(mockRes.setHeader).toHaveBeenCalledWith('x-request-id', 'header-req-id');
  });
  ...
  it('should fallback to x-correlation-id header if req.id and x-request-id are not present', () => {
    ...
    expect(mockRes.setHeader).toHaveBeenCalledWith('x-request-id', 'correlation-req-id');
  });
